...
def get_edit_user_kb() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="💰 Баланс"), KeyboardButton(text="👥 Рефералы")], # Fixed: Keyboard to KeyboardButton
...
async def create_crypto_bot_check(user_id: int, amount_usdt: float) -> dict:
    ...
    try:
        ...
        print("Crypto Bot API Response (createCheck):", response_data) # Debug
...
async def create_crypto_bot_invoice(user_id: int, amount_usdt: float) -> dict:
    ...
    try:
        ...
        response_data = response.json()
        print("Crypto Bot API Response (createInvoice):", response_data) # Debug
        return response_data
    except Exception as e:
...
async def check_invoice_status(invoice_id: int) -> dict:
    ...
    try:
        ...
        response_data = response.json()
        print(f"Crypto Bot API Response (check_invoice_status for {invoice_id}):", response_data) # Debug
        return response_data
    except Exception as e:
...
async def process_deposit(user_id: int, amount_usdt: float):
    ...
    if not invoice:
        print("DEBUG: Invoice creation failed (no response).") # Debug
        return False, "Ошибка при подключении к платежной системе"

    if not invoice.get('ok', False):
        error_msg = invoice.get('error', {}).get('name', 'Неизвестная ошибка')
        print(f"DEBUG: Invoice creation API error: {error_msg}") # Debug
        return False, f"Ошибка платежной системы: {error_msg}"

    if not invoice.get('result'):
        print("DEBUG: Invoice creation API response has no 'result'.") # Debug
        return False, "Платежная система не предоставила данные для оплаты"

    invoice_id = invoice['result']['invoice_id']
    print(f"DEBUG: Successfully obtained invoice_id: {invoice_id}") # Debug this!
    ...
    print(f"DEBUG: Constructed compact_pay_url: {compact_pay_url}") # Debug this!

    return True, {
        'pay_url': compact_pay_url,
...
async def check_payment_status(user_id: int, invoice_id: int, amount_usdt: float):
    ...
    while attempt < max_attempts:
        ...
        # Ensure that 'items' key exists and is a list before accessing
        if not invoice_data.get('result') or not isinstance(invoice_data['result'].get('items'), list) or not invoice_data['result']['items']:
            print(f"DEBUG: 'items' not found or empty for invoice {invoice_id}")
            attempt += 1
            continue
